...
@alpha.on_cmd("sudo", about={'header': "enable / disable sudo access"}, allow_channels=False)
...
@alpha.on_cmd("vsudo", about={'header': "lihat pengguna sudo"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if '-all' in message.flags:
        ...
        tmp_ = []
        for c_d in list(alpha.manager.enabled_commands):
...
@alpha.on_cmd("vscmd", about={'header': "lihat sudo cmds"}, allow_channels=False)
