...
async def kang_(message: Message):
    ...
    if replied and replied.media:
        ...
        photo = await alpha.download_media(message=replied, file_name=Config.DOWN_PATH)
    else:
        await message.edit("`ga bisa njir...`")
        return
    if photo:
        ...
        if exist is not False:
            async with alpha.conversation("Stickers", limit=30) as conv:
                try:
                ...
                while limit in msg.text:
                    pack += 1
                    packname = f"a{user.id}_by_alpha_{pack}"
                    packnick = f"{custom_packnick} Vol.{pack}"
                    if is_anim:
                        packname += "_anim"
                        packnick += " (Animated)"
            ...
            await message.edit("`Membuat pack baru tunggu sebentar dulu ya asu...`")
            async with alpha.conversation("Stickers") as conv:
                try:
                    await conv.send_message(cmd)
                except YouBlockedUser:
                    await message.edit("first **unblock dulu** @Stickers")
                ...
                if "Sorry, the file type is invalid." in rsp.text:
                    await message.edit(
                        "`Gagal anjing kasian deh, use` @Stickers "
                        "`bot to add the sticker manually.`"
                    )
                    return
                await conv.send_message(emoji_)
