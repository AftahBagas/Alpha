...
from alpha.utils import post_to_telegraph
...
async def telegraph_(message: Message):
    ...
    if (replied.text
        ...
        if replied.document:
            ...
            if not header:
                header = "Pasted content by @TheAlphaSupport"
            os.remove(dl_loc)
        else:
        ...
        jv_text = f"**[Telegraph Link By Alpha Userbot]({t_url})**"
    ...
    if replied.sticker:
        ...
        img.save(f'{Config.DOWN_PATH}/alpha.png', 'png')
        os.remove(dl_loc)
        dl_loc = f'{Config.DOWN_PATH}/alpha.png'
    ...
    try:
        ...
        await message.edit(f"**[Telegraph Link By Alpha Userbot](https://telegra.ph{response[0]})**")
    finally:
        os.remove(dl_loc)
