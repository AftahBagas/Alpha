...
from alpha import alpha, Message, pool, filters, get_collection, Config
...
@alpha.on_cmd("play", about={'header': "play or add songs to queue"},
...
@alpha.on_cmd("helpvc", about={'header': "help for voice_call plugin"},
               trigger=Config.SUDO_TRIGGER, allow_private=False, check_client=True,
               filter_me=False, allow_bots=False)
@vc_chat
@check_enable_for_all
async def _help(msg: Message):
    ...
    commands = alpha.manager.enabled_plugins["voice_call"].enabled_commands
    cmds = []
    raw_cmds = []
...
📘 <b>Doc:</b>  <code>Alpha Voice-Call Plugin</code>\n\n"""
...
@vc_chat.on_network_status_changed
...
if alpha.has_bot:
    ...
    @alpha.bot.on_callback_query(filters.regex(r"vol\((.+)\)"))
    @check_cq_for_all
    async def vol_callback(cq: CallbackQuery):
        ...
        if arg.isnumeric():
            ...
            try:
                async with alpha.conversation(cq.message.chat.id, user_id=cq.from_user.id) as conv:
