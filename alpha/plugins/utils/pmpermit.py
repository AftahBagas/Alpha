...
from alpha import alpha, filters, Message, Config, get_collection
from alpha.utils import SafeDict
...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await alpha.get_users(userid)).unblock()
...
@alpha.on_filters(~allowAllFilter & filters.incoming & filters.private & ~filters.bot
                   & ~filters.me & ~filters.service & ~Config.ALLOWED_CHATS, allow_via_bot=False)
async def uninvitedPmHandler(message: Message):
    """ pm message handler """
    user_dict = await alpha.get_user_dict(message.from_user.id)
...
@alpha.on_filters(~allowAllFilter & filters.outgoing & ~filters.edited
...
if alpha.has_bot:
    @alpha.bot.on_callback_query(filters.regex(pattern=r"pm_allow\((.+?)\)"))
    ...
    @alpha.bot.on_callback_query(filters.regex(pattern=r"pm_block\((.+?)\)"))
    ...
    @alpha.bot.on_callback_query(filters.regex(pattern=r"^pm_spam$"))
    async def pm_spam_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await c_q.edit_message_text(
                blocked_message.format_map(SafeDict(**user_dict)))
            await alpha.block_user(c_q.from_user.id)
            await asyncio.sleep(1)
            await CHANNEL.log(
                f"#BLOCKED\n{c_q.from_user.mention} has been blocked due to spamming in pm !! ")

    @alpha.bot.on_callback_query(filters.regex(pattern=r"^pm_contact$"))
