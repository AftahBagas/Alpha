...
from alpha import Config, Message, alpha, filters, get_collection
...
async def gmute_user(msg: Message):
    ...
    if not reason:
        await msg.edit(
            ...
            "`Dibatalkan karena Tidak ada alasan GMute oleh Pengguna`",
            del_in=5,
        )
        return
    user_id = get_mem["id"]
    ...
    if user_id in Config.SUDO_USERS:
        await msg.edit(
            ...
            "**Tip:** `Hapus mereka dari Sudo Daftar dan coba lagi. (¬_¬)`",
            del_in=5,
        )
        return
