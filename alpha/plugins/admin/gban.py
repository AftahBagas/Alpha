...
from alpha import Config, Message, alpha, filters, get_collection, pool
...
async def gban_user(message: Message):
    ...
    if not reason:
        await message.edit(
            ...
            "Syntax Ellol kasih alasan nya tolol!",
            del_in=5,
        )
        return
    user_id = get_mem["id"]
    ...
    if user_id in Config.SUDO_USERS:
        await message.edit(
            ...
            "**Tip:** hapus dari sudo dulu baru gban ulang. (¬_¬)",
            del_in=5,
        )
        return
    ...
    if "chat_ids" in found:
        for chat_id in found["chat_ids"]:
            try:
                ...
                pass
    await message.edit(
        r"😁**Mengampuni manusia meresahkan!**😁"
        f"\n\n**Nama:** [{firstname}](tg://user?id={user_id})\n"
        f"**User ID:** `{user_id}`"
    )
