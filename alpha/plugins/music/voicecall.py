...
from alpha import Config, Message, get_collection, pool, userge
from alpha.plugins.tools.alive import _parse_arg
from alpha.plugins.tools.utube_inline import BASE_YT_URL, get_yt_video_id, get_ytthumb
from alpha.plugins.misc.upload import check_thumb
from alpha.plugins.utils.songlink import find_url_from_msg, get_song_link
...
class Alpha(GroupCall):
    def __init__(self, chat_id: int):
        ...
        super().__init__(
            client=alpha, play_on_repeat=self.replay_songs, path_to_log_file=""
        )
...
vc_chats: Dict[int, Alpha] = {}


async def get_groupcall(chat_id: int) -> Alpha:
    if not vc_chats.get(chat_id):
        group_call = vc_chats[chat_id] = Alpha(chat_id)
        ...
        if alpha.has_bot:
            try:
                await alpha.get_chat_member(
                    chat_id, (await alpha.bot.get_me()).username
                )
...
async def play_now(gc: Alpha) -> None:
    r = gc.playlist[0]
    key = r["id"]
    client = alpha.bot if r["has_bot"] else alpha
...
async def get_rawaudio_thumb(data: Dict) -> Optional[tuple]:
    key = data["id"]
    msg = data["msg"]
    client = alpha.bot if data["has_bot"] else userge
...
async def append_playlist(gc: Alpha, m: Message, media_grp: bool, **kwargs) -> None:
    thumb = kwargs["thumb"]
    title = kwargs["title"]
    gc.playlist.append(
        {
...
if userge.has_bot:
    ...
    @alpha.bot.on_callback_query(filters.regex(pattern=r"^vcbtn_([a-z]+)$"))
    @check_owner
    ...
    @alpha.bot.on_callback_query(filters.regex(pattern=r"^vc_([a-z]+)_([-a-z+]+)$"))
    @check_owner
